...
for turn in permutations(player, 8):
    turn = list(turn)
    turn=turn[:3] + [0] + turn[3:] # 1번선수 4번타자로
    score=0
    idx=0
    for inning in range(1, n+1):
        ...
        while out_cnt<3:
            if scr[inning-1][turn[idx]]==0:
                out_cnt+=1
            elif scr[inning-1][turn[idx]]==1:
                score+= base_3
                base_1, base_2, base_3=1, base_1, base_2
            elif scr[inning-1][turn[idx]]==2:
                score+=base_2+base_3
                base_1, base_2, base_3=0, 1, base_1
            elif scr[inning-1][turn[idx]]==3:
                score+=base_1+base_2+base_3
                base_1, base_2, base_3=0, 0, 1
            elif scr[inning-1][turn[idx]]==4:
